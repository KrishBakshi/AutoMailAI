...
def bulk_output_to_docx(excel, pdf, template, progress: gr.Progress = gr.Progress(track_tqdm=True)):
...
def bulk_output_to_gmail(excel, pdf, template, progress: gr.Progress = gr.Progress(track_tqdm=True)):
    # Check for uploaded files
    if excel is None:
        return gr.Error("No Excel file uploaded.")
    if pdf is None:
    ...
    for _, row in tqdm(df.iterrows(), total=len(df), desc="Generating emails"):
        ...
        try:
            email = utils.generate_email(prompt, selected_template)
            utils.save_gmail_draft(email, pdf.name)
        except Exception as e:
            return gr.Error(f"Error generating Gmail draft for {row['Company Name']}: {str(e)}")

    return "Emails have been drafted as Gmail drafts."
        ...
        with gr.TabItem("Single Email"):
            with gr.Row():
                ...
                with gr.Column(scale=1):
                    ...
                    with gr.Row():
                        with gr.Column(scale=1):
                            write_btn = gr.Button("Draft Word Document")
            ...
            draft_btn.click(utils.save_gmail_draft, inputs=[state, pdf_input], outputs=[output_box])
        ...
        with gr.TabItem("Batch Email"):
            with gr.Row():
                with gr.Column(scale=1):
                    ...
                    with gr.Row():
                        template = gr.Dropdown(choices=list(config.template.keys()), label="Select Email Template", interactive=True)
                    docx_btn = gr.Button("Generate Word Documents")
                    gmail_btn = gr.Button("Draft Gmail Emails")
                with gr.Column(scale=1):
                    output_box = gr.Textbox(label="Output", lines=10, autoscroll=True)
                    with gr.Row():
                        with gr.Column(scale=1):
            ...
            docx_btn.click(bulk_output_to_docx, inputs=[excel_input, pdf_input, template], outputs=[output_box])
            gmail_btn.click(bulk_output_to_gmail, inputs=[excel_input, pdf_input, template], outputs=[output_box])
